...
def make_0based_vocab(somearray):
    vocab = []
    ...
    new_version = []
    for thing in somearray:
        new_version.append(vocab.index(thing))
    return new_version
...
#def get_Text8(which_set):
    #if which_set not in _data_cache:
        #try:
            #data = np.load('text8.npz') #np.load('/data/lisa/data/PennTreebankCorpus/char_level_penntree.npz')
        #except:
            #FileNotFoundError('Could not load text8')
        ##data = np.load(path)
        ## put the entire thing on GPU in one-hot (takes
        ## len(self.vocab) * len(self.data) * sizeof(floatX) bytes
        ## which is about 1G for the training set and less for the
        ## other sets)
        ##cudandarray = theano.sandbox.cuda.cuda_ndarray.cuda_ndarray.CudaNdarray
        ##cudandarray = theano.sandbox.cuda.CudaNdarray
        ## (doing it in numpy first because cudandarray doesn't accept
        ## lists of indices)
        #one_hot_data = np.eye(27, dtype=theano.config.floatX)[make_0based_vocab(np.int64(data[which_set]))]
...
class Text8(fuel.datasets.Dataset):
    ...
    def __init__(self, which_set, length, augment=False):
        ...
        self.augment = augment
        try:
            data = np.load("/data/lisatmp3/cooijmat/datasets/text8/text8.npz")
        except:
            raise FileNotFoundError("Could not load text8 dataset")
...
def get_text8_stream(which_set, batch_size, length, drop_prob_states, drop_prob_cells, drop_prob_igates,
                   hidden_dim, for_evaluation, num_examples=None,
                   augment=True):
