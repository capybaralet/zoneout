...
from utils_DATA import get_ptb_stream
...
from recurrent import MEMORY_CELL
from recurrent import ZoneoutLSTM, ZoneoutGRU, ZoneoutSimpleRecurrent
...
def parse_args():
    ...
    parser.add_argument('--initialization', choices=['glorot', 'uniform', 'identity', 'ortho'],
                        default='ortho', help='Weight initialization method')
    ...
    parser.add_argument('--z_prob_cells',
                        ...
                        default=1.0,
                        help='(for rnn with non-shared mask) Despite the name, is the update probability. Sorry.' )
    parser.add_argument('--drop_igates',
                        type=float,
                        default=1.0,
...
def train(step_rule, layer_size, epochs, seed, experiment_path, initialization, weight_noise,
          to_watch, patience, z_prob, z_prob_states, z_prob_cells, drop_igates, ogates_zoneout, 
          batch_size, stoch_depth, share_mask, gaussian_drop, rnn_type, num_layers, 
          norm_cost_coeff, penalty, seq_len, input_drop,
          **kwargs):
    ...
    train_stream_evaluation = get_ptb_stream(
        'train', batch_size, seq_len, z_prob_states, z_prob_cells, z_prob_igates, layer_size, True)
    dev_stream = get_ptb_stream(
        'valid', batch_size, seq_len, z_prob_states, z_prob_cells, z_prob_igates, layer_size, True)
    ...
    if rnn_type.lower()=='lstm':
        ...
        recurrent_layer = ZoneoutLSTM(dim=layer_size, weights_init=weights_init, activation=Tanh(), model_type=6, name='rnn', ogates_zoneout=ogates_zoneout)
    elif rnn_type.lower()=='gru':
        in_to_hid = Linear(50, layer_size*3, name='in_to_hid',
                       weights_init=weights_init, biases_init=Constant(0.0))
        recurrent_layer = ZoneoutGRU(dim=layer_size, weights_init=weights_init, activation=Tanh(), name='rnn')
    elif rnn_type.lower()=='srnn': #FIXME!!! make ReLU
        in_to_hid = Linear(50, layer_size, name='in_to_hid',
                       weights_init=weights_init, biases_init=Constant(0.0))
        recurrent_layer = ZoneoutSimpleRecurrent(dim=layer_size, weights_init=weights_init, activation=Rectifier(), name='rnn')
    else:
